...
static int group_cipher_get_sender_key_chat(group_cipher *cipher, sender_message_key **sender_key, sender_key_state *state, uint32_t iteration);
                         ...
                         signal_buffer **plaintext)
{
    ...
    result = group_cipher_get_sender_key_chat(cipher, &sender_key, state, sender_key_message_get_iteration(ciphertext));
    if(result < 0) {
        goto complete;
    }
...
int group_cipher_get_sender_key_chat(group_cipher *cipher, sender_message_key **sender_key, sender_key_state *state, uint32_t iteration)
{
    int result = 0;
