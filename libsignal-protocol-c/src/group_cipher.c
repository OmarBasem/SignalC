...
static int group_cipher_get_sender_key(group_cipher *cipher, sender_message_key **sender_key, sender_key_state *state, uint32_t iteration, int isChat);
                         ...
                         signal_buffer **plaintext, int isChat)
{
    ...
    result = group_cipher_get_sender_key(cipher, &sender_key, state, sender_key_message_get_iteration(ciphertext), isChat);
    if(result < 0) {
        goto complete;
    }
...
int group_cipher_get_sender_key(group_cipher *cipher, sender_message_key **sender_key, sender_key_state *state, uint32_t iteration, int isChat)
{
    int result = 0;
    ...
    if(sender_chain_key_get_iteration(chain_key) > iteration) {
        if(sender_key_state_has_sender_message_key(state, iteration)) {
            result_key = sender_key_state_remove_sender_message_key(state, iteration, isChat);
            if(!result_key) {
